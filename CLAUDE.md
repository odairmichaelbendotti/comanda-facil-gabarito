...
- `app/lib/mock-users.ts` tem a lista de usuários mockados (não há backend ainda) e o tipo `UserRole` (`"admin" | "garcom" | "cozinha"`). `app/lib/store/auth-store.ts` é a store Zustand (`useAuthStore`) com `user`, `isAuthenticated`, `login(cpf, senha)`, `logout()` — usa `persist` (localStorage) para manter a pessoa logada ao recarregar a página, e expõe `hasHydrated` para saber quando a leitura do localStorage já terminou.
...
- `ProtectedRoute` (`app/components/ProtectedRoute.tsx`) é o componente que aplica a proteção — página privada envolve seu conteúdo com `<ProtectedRoute>{...}</ProtectedRoute>` (veja `/pedidos`, `/produtos`, `/configuracoes`, `/funcionarios`). Ele lê a rota atual, consulta `ROUTE_ACCESS` sozinho (a página não passa papéis manualmente): sem sessão carregada ainda ou papel não permitido, mostra `PageLoadingState`/`AccessDenied`; sem login, redireciona para `/login?redirect=<rota>`, que depois do login volta para essa mesma rota; com o papel permitido, renderiza o conteúdo. `/preview` e `/login` ficam fora dessa proteção (públicas).
